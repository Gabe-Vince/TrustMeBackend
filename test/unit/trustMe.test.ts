...
import {formatEther, getContractAddress, id, parseEther} from 'ethers/lib/utils';
import {deployments, ethers} from 'hardhat';
import {BuyerToken, SellerToken, BuyerNFT, SellerNFT} from '../../typechain/contracts';
import {TrustMeLib} from '../../typechain';
const {waffle} = require('hardhat');
import {time} from '@nomicfoundation/hardhat-network-helpers';
import {log} from 'console';

describe('TrustMeLib Contract Tests', () => {
	let signers: SignerWithAddress[];
	let trustMe: TrustMeLib;
	let buyerToken: BuyerToken;
	let sellerToken: SellerToken;
	let sellerNFT: SellerNFT;
	let buyerNFT: BuyerNFT;
	...
	beforeEach(async () => {
		...
		trustMe = await ethers.getContract('TrustMeLib');
		buyerToken = await ethers.getContract('BuyerToken');
		sellerToken = await ethers.getContract('SellerToken');
		sellerNFT = await ethers.getContract('SellerNFT');
		buyerNFT = await ethers.getContract('BuyerNFT');
		...
		const tx2 = await sellerNFT.mint(seller.address);
		await tx2.wait();
		console.log('buyerToken', buyerToken.address);
		console.log(ethers.constants.AddressZero);
	});

	describe('addTrade Functionailty', () => {
		it('should add a trade', async () => {
			const trade = {
				...
				nft: {
					senderAddressNFT: ethers.constants.AddressZero,
					senderTokenIdNFT: 0,
					receiverAddressNFT: ethers.constants.AddressZero,
					receiverTokenIdNFT: 0,
				},
				token: {
					senderToken: ethers.constants.AddressZero,
					senderAmountToken: 0,
					receiverToken: buyerToken.address,
					receiverAmountToken: 10,
				},
				eth: {
					senderAmountETH: 1,
					receiverAmountETH: 0,
			...
			const tx = await trustMe.connect(seller).addTrade(trade, {value: trade.eth.senderAmountETH});

			await tx.wait();
			const tradeId = await trustMe.getTradesForUser(seller.address);
			const tradeBlock = await trustMe.getTrade(tradeId[0]);

			console.log(tradeBlock.sender === seller.address);
		});
	});
});
